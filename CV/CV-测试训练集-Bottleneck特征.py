# -*- coding: utf-8 -*-
"""
Created on Sat Sep 17 14:06:02 2016
...
path1 = 'F:\CV大赛\测试集\Testset 1'
answer1 = pd.read_csv('F:\CV大赛\测试集\Answer\BOT_Image_Testset 1.txt', header='infer', delimiter='\t')
path2 = 'F:\CV大赛\测试集\Testset 2'
answer2 = pd.read_csv('F:\CV大赛\测试集\Answer\BOT_Image_Testset 2.txt', header='infer', delimiter='\t')
path3 = 'F:\CV大赛\测试集\Testset 3'
answer3 = pd.read_csv('F:\CV大赛\测试集\Answer\BOT_Image_Testset 3.txt', header='infer', delimiter='\t')
path4 = 'F:\CV大赛\测试集\Testset 4'
answer4 = pd.read_csv('F:\CV大赛\测试集\Answer\BOT_Image_Testset 4.txt', header='infer', delimiter='\t')

nb_classes = 12
pics = [] #保存图片的完整文件路径
labels = []
for path, answer in [(path1, answer1), (path2, answer2), (path3, answer3), (path4, answer4)]:
	for pic in tqdm(os.listdir(path)):
		pics.append(os.path.join(path, pic))
		label = int(answer[answer.icol(0)==pic[:32]]['Top1'])		
		labels.append(label)
pics = np.array(pics)		
...
train = tables.open_file('Test_train.h5', 'w')
test = tables.open_file('Test_test.h5', 'w')
