...
#生成模型
def VGG_16(weight='0921weights_Test.h5'):
	model = Sequential()
...
for item in answer.iterrows(): #item是长度为2的tuple
	...
	if top2_score == 0.4: #top1错误， top2正确
		error1[item[0]] = []
		error1[item[0]].append(animal[y_true])
		error1[item[0]].append(animal[top1])
	elif top2_score == 0.0: #	top1、2都错误	
		error2[item[0]] = []
		error2[item[0]].append(animal[y_true])
		error2[item[0]].append(animal[top1])
	elif top2_score in [0.8, 2.0, 1.4, 2.8]:
		hidden[item[0]] = []
		hidden[item[0]].append(animal[y_true])
		hidden[item[0]].append(animal[top1])
...
#把分类错误的图片提取到文件夹
source = 'F:\CV大赛\测试集\Testset %d' % k
error = r'F:\CV大赛\测试集\error\5'

#命名方式：图片名称-正确-错误
for img in os.listdir(source):
	if img[:32] in error1.keys():
		file = error1[img[:32]]
		copyfile(os.path.join(source, img), os.path.join(error, 'error1', file[0]+'-'+file[1]+'-'+img)) 
	elif img[:32] in error2.keys():
		file = error2[img[:32]]
		copyfile(os.path.join(source, img), os.path.join(error, 'error2', file[0]+'-'+file[1]+'-'+img))
	elif img[:32] in hidden.keys():
		file = hidden[img[:32]]
		copyfile(os.path.join(source, img), os.path.join(error, 'hidden', file[0]+'-'+file[1]+'-'+img))
